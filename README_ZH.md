...
- **📖 长文本 TTS** – 合成 4 分钟以上的长文本，自动分块 + 语音锚定（从第一个批次捕获固定声学参考，防止跨批次内容重复和音色漂移）+ 滑动窗口上下文 + 延迟解码 + 按批次自动计算 token 预算，保持声音一致性并降低显存
...
长文本语音合成，支持自动分块、语音锚定和滑动窗口上下文，在数分钟的音频中保持声音一致性。

文本自动按句子边界分割为多个块。每个块处理时会参考前面块的内容（由 `max_context_batches` 控制），以保持声音质量的一致性。可选从参考音频克隆声音。

**语音锚定（Voice Anchoring）：**
从第一个批次中捕获最后约 2 秒的声学 token 作为固定的"语音锚点"，并包含在每个后续批次的助手上下文中。这防止了两个关键问题：
- **内容重复** — 没有锚点编码时，模型看到空的助手回合可能会重复之前批次的内容
- **音色漂移** — 锚点提供了一致的声学证据，防止音色在批次间重置或偏移

锚点是来自批次 0 的固定参考（不是滚动链式传递），因此质量不会随时间因误差累积而下降。它仅增加极少的显存（约 150 个编解码帧）。

**显存优化：**
- **自动精度检测：** RTX 20/GTX 16 系列（Turing）自动回退到 `float16` — 防止 8GB 显卡 OOM
- **语音锚点** — 来自第一个批次的固定声学参考，约 2 秒 VQ 编码（约 150 帧），防止内容重复和音色漂移，且不会累积质量损失
